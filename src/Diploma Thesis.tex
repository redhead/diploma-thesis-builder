...
Java, the code of that class could look like in listing \#X
\textbf{listing needed}.
...
An example of an event class is presented in listing X\textbf{listing
needed}.
...
aggregate root that extends this abstract class is in listing \#x
\textbf{listing needed}.
...
any objects not requiring to implement the interface. See listing X
\textbf{listing needed} for an example.
...
shown in Listing X\textbf{reference needed}.
...
fields). See the implementation in PRILOHA X \textbf{reference needed}.
The class for the event that gets emitted in the \texttt{update()}
method is presented in LISTING X \textbf{reference needed}.
...
\texttt{createLabel()} method presented in Listing X\textbf{reference
needed}. After refactoring, it only delegates the execution of business
...
The dispatched command, defined by the class in Listing
X\textbf{reference needed}, is forwarded to the command bus
...
\texttt{CreateLabelCommand} is presented in Listing X \textbf{reference
needed}.
...
a label with the handler for \texttt{LabelCreatedEvent} shown in Listing
X\textbf{reference needed}.
...
read model effectively). Listing X \textbf{reference needed} shows an
...
cases, like in the listing X\textbf{reference needed} above, throwing of
an expected exception is needed to assert.
